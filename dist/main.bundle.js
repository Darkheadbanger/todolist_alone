...
var input = document.querySelector("input"); // On simule la base de donées en créant une variable qui comporte des objets dans un tableau
...
var displayTodo = function displayTodo() {
  ...
  var todosNode = todos.map(function (todo, index) {
    ...
    console.log("index", index); // On retourne la function createToDoElement qui comporte la création de todo list en function de resultat que la boucle map donne
  ...
  ul.innerHTML = ""; // Ensuite, on ajoute le résultat de la liste todolist qui a été itéré avec la boucle map (qui modifie ses elements) dans la page
...
var createToDoElement = function createToDoElement(todo) {
  ...
  var li = document.createElement("li"); // On ajoute via la propriété innerHTML le html complet de ce qu'on veut afficher
